...
test('request history excludes current and legacy cost footers while keeping tool results', () => {
  ...
  const ctx = vm.createContext({ i: { pinned: [] }, CHEATSHEET: '', MCP_CONTRACT_TEXT: '', todayIndiana: () => '2026-09-17', u: { verified: 'fixture' }, gitStatus: null, task: null });
  ...
  /* the same builder, with git and a task in hand, carries both */
  const gctx = vm.createContext({ i: { pinned: [] }, CHEATSHEET: '', MCP_CONTRACT_TEXT: '', todayIndiana: () => '2026-09-17', u: { verified: 'fixture' },
    ...
    task: { goal: 'fixture goal', steps: [{ text: 'one', done: true }, { text: 'two', done: false }] } });
  ...
  assert.match(String(gmessages[0].content), /## TASK: fixture goal\n- \[x\] one\n- \[ \] two/);
});
