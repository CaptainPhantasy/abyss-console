...
|---|---|---|---|---|
| 1 | **The helper ran any command a web page asked for.** CORS was `*` and `/run` checked only `confirm:true`; a page from another origin executed a command and a marker file appeared on disk. | A per-install token required on every privileged route (`/run`, `/fs/write`, `/fs/patch`, `/fs/undo`, `/lib/*`, `/room/*`, `/mcp`); the token is injected only into the page the helper serves, plus an origin allowlist that refuses other browser origins. | `node --test tests/helper-auth.e2e.mjs` — foreign origin refused, served page works end to end | open |
| 2 | **The secret scrubber had a bypass.** It ran on the composed message only, so pinned text and tool results reached the model unscrubbed. | One choke point at request assembly, covering the system message, pinned text, every tool result, attachments and the draft. | a test that pins a file containing `password=hunter2hunter2` and asserts the outgoing body carries neither | open |
