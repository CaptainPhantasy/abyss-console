...
/* Closing the tab asks the helper to stop — only when the helper served this page (it put the
   token in). The helper waits a couple of seconds before stopping, so a reload is not a kill;
   launchd starts it again on the next visit, so nothing is left running for you. */
...
function WriteReview({ blocks, onNote }) {
  ...
  const compare = async (i) => {
    ...
    try {
      ...
      patch(i, { state: "ready", diff: { added, removed, rows: shown, hidden: Math.max(0, out.length - shown.length), existed: before !== "", beforeLines: beforeLines.length } });
    } catch (err) {
      patch(i, { state: "error" });
  ...
  const write = async (i) => {
    const d2 = rows[i].diff;
    if (d2 && d2.existed && d2.beforeLines > 200 && d2.removed > 20 && d2.removed > d2.added && rows[i].state !== "armed") {
      patch(i, { state: "armed" });
      onNote(rows[i].path + " has " + d2.beforeLines + " lines on disk and this whole-file write would remove " + d2.removed + " of them. Press write again if you mean it, or send a diff instead.");
      return;
    }
            ...
            children: [
              jsxRuntime.jsxs("div", { style: { display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }, children: [
                ...
                jsxRuntime.jsx("span", { style: { marginLeft: "auto", display: "flex", gap: 6 } , children: [
                  ...
                  jsxRuntime.jsx("button", { style: STYLES.microBtn, disabled: r.state === "writing", onClick: () => write(i), children: r.state === "written" ? "written ✓" : r.state === "armed" ? "write anyway (removes " + ((r.diff && r.diff.removed) || 0) + ")" : "write" }),
...
function App() {
  const [e, t] = React.useState("chat"),
    ...
    [libPath, setLibPath] = React.useState(""),
    ...
    [stalePins, setStalePins] = React.useState([]),
    [projectStale, setProjectStale] = React.useState(false),
    [gitStatus, setGitStatus] = React.useState(null),
  ...
  /* coming back to the tab re-checks the map and the pins against the disk */
  React.useEffect(() => {
    const onFocus = () => { checkFresh(); };
    ...
    Tn = (h, z) => {
      ...
      const gitText =
        gitStatus && gitStatus.ok
          ? "\n\n## GIT: branch " + gitStatus.branch + (gitStatus.head ? " at " + gitStatus.head : "") + " · " +
            (gitStatus.clean
              ? "no uncommitted changes"
              : gitStatus.changed.length + " uncommitted change" + (gitStatus.changed.length === 1 ? "" : "s") + ": " +
...
` +
    ...
    sendMessage = async (h, reviewOf) => {
      ...
      try {
        for (let step = 0; step < 8; step++) {
          ...
          /* one independent read must not wait on another: consecutive read-only calls are
             dispatched together; every result still lands in its own row, in call order */
          const readToolNames = ["read_project_files", "search_project", "find_references"];
          const withDepth = async (work) => {
            const m2 = globalThis.__ABYSS_TOOLS || (globalThis.__ABYSS_TOOLS = { now: 0, peak: 0 });
            m2.now++;
            m2.peak = Math.max(m2.peak, m2.now);
            try {
              return await work();
            } finally {
              m2.now--;
          ...
          const landTool = (te, Fe) => {
            ...
            x((Fr) => {
              const Rn = [...Fr];
              const at2 = Rn.findIndex((z2) => z2.role === "tool" && z2.toolCallId === te.id);
              if (at2 !== -1) {
                Rn[at2] = {
                  ...Rn[at2],
                  status: Fe.status,
          ...
          for (let at2 = 0; at2 < F.toolCalls.length; ) {
            const te = F.toolCalls[at2];
            let ne = {};
            try {
              ne = JSON.parse(te.function.arguments || "{}");
            } catch {}
            if (readToolNames.indexOf(te.function.name) !== -1) {
              const batch = [];
              while (at2 < F.toolCalls.length && readToolNames.indexOf(F.toolCalls[at2].function.name) !== -1) {
                const t2 = F.toolCalls[at2];
                let a2 = {};
                try {
                  a2 = JSON.parse(t2.function.arguments || "{}");
                } catch {}
                batch.push({ te: t2, ne: a2 });
                at2++;
              }
              const results = await Promise.all(batch.map((b2) => withDepth(() => executeTool(b2.te, b2.ne))));
              for (let i2 = 0; i2 < batch.length; i2++) landTool(batch[i2].te, results[i2]);
            } else {
              landTool(te, await withDepth(() => executeTool(te, ne)));
              at2++;
            }
          }
          if (step === 7) {
    ...
    indexProject = async (path) => {
      ...
      try {
        ...
        const map =
          ...
          "\n\n" +
          shown.join("\n");
        setProject({ root: tree.root, count: tree.count, bytes: tree.bytes, sendable: sendable.length, tokens, files: tree.files, indexedAt: Date.now(), newest: tree.newest || 0 });
        setProjectStale(false);
        await refreshGit(tree.root);
    ...
    /* the whole library in one file, and back again */
    exportLibrary = () => {
      try {
        const data = { exportedAt: new Date().toISOString(), sessions, recipes };
        const blob = new Blob([JSON.stringify(data, null, 2)], { type: "application/json" });
        const a2 = document.createElement("a");
        a2.href = URL.createObjectURL(blob);
        a2.download = "abyss-library-" + new Date().toISOString().slice(0, 10) + ".json";
        a2.click();
        URL.revokeObjectURL(a2.href);
        setSessionMsg("exported " + sessions.length + " session(s) and " + recipes.length + " recipe(s) as json");
      } catch {
        setSessionMsg("the browser refused the download");
      }
    },
    importLibrary = async () => {
      const p2 = libPath.trim();
      if (!p2) { setSessionMsg("give the path of a library json first"); return; }
      try {
        const r = await (await fetch(HELPER_URL + "/fs/read?path=" + encodeURIComponent(p2))).json();
        if (typeof r.text !== "string") { setSessionMsg("could not read that file: " + (r.error || "the helper said no")); return; }
        let data = null;
        try { data = JSON.parse(r.text); } catch { setSessionMsg("that file is not json"); return; }
        const inSessions = Array.isArray(data && data.sessions) ? data.sessions.filter((x2) => x2 && x2.id) : [];
        const inRecipes = Array.isArray(data && data.recipes) ? data.recipes.filter((x2) => x2 && x2.id) : [];
        if (!inSessions.length && !inRecipes.length) { setSessionMsg("nothing to import — that file has no sessions and no recipes"); return; }
        for (const s2 of inSessions) {
          try { await fetch(HELPER_URL + "/lib/sessions", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(s2) }); } catch {}
        }
        for (const r2 of inRecipes) {
          try { await fetch(HELPER_URL + "/lib/recipes", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(r2) }); } catch {}
        }
        await loadLibraryFromDisk();
        setLibPath("");
        setSessionMsg("imported " + inSessions.length + " session(s) and " + inRecipes.length + " recipe(s)");
      } catch {
        setSessionMsg("the helper is not answering");
      }
    },
    ...
    addRecipe = async () => {
      ...
      const entry = { id: "r" + Date.now(), name, text };
      const next = [entry, ...recipes].slice(0, 200);
      setRecipes(next);
      await storageSet(STORAGE_KEYS.recipes, next);
      try {
        await fetch(HELPER_URL + "/lib/recipes", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify(entry),
        });
    ...
    /* ---- staleness: the map and the pins versus what is on disk now -------- */
    checkPins = async () => {
      const changed = [];
      for (const p2 of i.pinned || []) {
        if (!p2.path) continue;
        try {
          const r = await (await fetch(HELPER_URL + "/attach?path=" + encodeURIComponent(p2.path))).json();
          if (r.ok && typeof r.text === "string" && r.text !== p2.text) changed.push(p2.id);
        } catch {}
      }
      setStalePins(changed);
    },
    rePin = async (id) => {
      const p2 = (i.pinned || []).find((x2) => x2.id === id);
      if (!p2 || !p2.path) return;
      try {
        const r = await (await fetch(HELPER_URL + "/attach?path=" + encodeURIComponent(p2.path))).json();
        if (!r.ok || typeof r.text !== "string") {
          setProjectMsg("could not re-read " + (p2.name || "that file") + ": " + (r.error || "the helper said no"));
          return;
        }
        K({ pinned: (i.pinned || []).map((x2) => (x2.id === id ? { ...x2, text: r.text, tokens: r.tokens || Math.ceil(r.text.length / 4), at: new Date().toISOString() } : x2)) });
        setStalePins((prev) => prev.filter((x2) => x2 !== id));
        setProjectMsg("re-read " + p2.name + " — the pin now holds the file as it is on disk");
      } catch {
        setProjectMsg("the helper is not answering");
      }
    },
    /* ---- git: status, diff, stage and commit go through the helper ---------- */
    refreshGit = async (root2) => {
      const where = root2 || (project && project.root);
      if (!where) { setGitStatus(null); setGitMsg(""); return; }
      try {
        const r = await (await fetch(HELPER_URL + "/git/status?path=" + encodeURIComponent(where))).json();
        setGitStatus(r && r.ok ? r : null);
        setGitMsg(r && r.ok ? "" : (r && r.error) || "the helper is not answering");
      } catch { setGitMsg("the helper is not answering"); }
    },
    showGitDiff = async (file, staged) => {
      if (!project) return;
      try {
        const r = await (await fetch(HELPER_URL + "/git/diff?path=" + encodeURIComponent(project.root) + "&file=" + encodeURIComponent(file) + (staged ? "&staged=1" : ""))).json();
        setGitDiff(r && r.ok
          ? { file, staged: !!staged, text: r.text || "(no difference against HEAD)", truncated: !!r.truncated }
          : { file, staged: !!staged, text: (r && r.error) || "could not read the diff", truncated: false });
      } catch { setGitMsg("the helper is not answering"); }
    },
    stageFiles = async (files) => {
      if (!project) return;
      try {
        const r = await (await fetch(HELPER_URL + "/git/stage", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: project.root, files, confirm: true }) })).json();
        setGitMsg(r && r.ok ? "staged " + (files && files.length ? files.join(", ") : "everything") : "could not stage: " + ((r && r.error) || "?"));
        await refreshGit();
      } catch { setGitMsg("the helper is not answering"); }
    },
    commitChanges = async () => {
      ...
      if (!message) { setGitMsg("a commit needs a message"); return; }
      try {
        const r = await (await fetch(HELPER_URL + "/git/commit", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ path: project.root, message, confirm: true }) })).json();
        if (r && r.ok) {
          setGitMsg("committed " + r.hash + " — " + message);
          setGitCommitMsg("");
          setGitDiff(null);
          await refreshGit();
        } else setGitMsg("could not commit: " + ((r && r.error) || "?"));
      } catch { setGitMsg("the helper is not answering"); }
    },
    checkFresh = async () => {
      if (project && project.root) {
        try {
          const t2 = await (await fetch(HELPER_URL + "/fs/tree?limit=4000&path=" + encodeURIComponent(project.root))).json();
          if (t2 && !t2.error) setProjectStale((t2.newest || 0) > (project.newest || 0));
        } catch {}
        await refreshGit(project.root);
      }
      await checkPins();
    },
    triageTrace = () => {
                ...
                (() => {
                  ...
                  if (patch) {
                    return jsxRuntime.jsx("div", {
                      ...
                      children: jsxRuntime.jsx(PatchReview, { found: patch, onNote: (msg) => { M(msg); setProjectMsg(msg); }, key: m.length }),
                    });
                  }
                  return blocks.length
                    ? jsxRuntime.jsx("div", {
                        style: { padding: "0 26px 8px" },
                        children: jsxRuntime.jsx(WriteReview, { blocks, onNote: (msg) => { M(msg); setProjectMsg(msg); }, key: m.length }),
                      })
                    : null;
                })(),
              e === "chat" &&
                  ...
                  children: [
                    ...
                    jsxRuntime.jsxs("div", {
                      ...
                      children: [
                        ...
                        jsxRuntime.jsx("span", {
                          style: { color: projectStale ? "var(--amber)" : "var(--foam)", fontFamily: "'JetBrains Mono',monospace", whiteSpace: "nowrap" },
                          children: project ? project.root.split("/").filter(Boolean).pop() + " · " + project.count + " files" + (projectStale ? " · files changed — press re-index" : "") : "no project indexed",
                        }),
                        jsxRuntime.jsxs("span", {
                          style: { marginLeft: "auto", color: "var(--kelp)", fontFamily: "'JetBrains Mono',monospace", whiteSpace: "nowrap" },
                          children: [
                      ...
                      jsxRuntime.jsxs(jsxRuntime.Fragment, {
                        ...
                        jsxRuntime.jsxs("div", {
                          ...
                          children: [
                            ...
                            jsxRuntime.jsx("button", { style: STYLES.ghostBtn, title: "download every saved session and recipe as one json file", onClick: exportLibrary, children: "export library" }),
                            jsxRuntime.jsx("input", {
                              style: { ...STYLES.input, width: 210 },
                              placeholder: "import library from this path…",
                              value: libPath,
                              onChange: (h) => setLibPath(h.target.value),
                              onKeyDown: (h) => h.key === "Enter" && importLibrary(),
                            }),
                            jsxRuntime.jsx("button", { style: STYLES.ghostBtn, title: "read a library json and merge it in — the same id updates, new ids are added", onClick: importLibrary, children: "import library" }),
                            sessionMsg || libNote
                              ? jsxRuntime.jsx("span", {
                                  style: { color: "var(--kelp)", fontFamily: "'JetBrains Mono',monospace", whiteSpace: "nowrap" },
                          ...
                          children: [
                            jsxRuntime.jsx("span", {
                              ...
                              children:
                                "git · " +
                                (gitStatus && gitStatus.ok
                                  ? gitStatus.branch + " at " + gitStatus.head + (gitStatus.clean ? " · clean" : " · " + gitStatus.changed.length + " changed")
                                  : gitMsg || "no repo in this project"),
                            }),
                            gitStatus && gitStatus.ok
                              ? jsxRuntime.jsx("button", { style: STYLES.ghostBtn, title: "ask git again", onClick: () => refreshGit(), children: "refresh" })
                              : null,
                            gitStatus && gitStatus.ok && !gitStatus.clean
                              ? jsxRuntime.jsx("button", { style: STYLES.ghostBtn, title: "stage every change — asks nothing, stages a lot; commit stays explicit", onClick: () => stageFiles([]), children: "stage all" })
                              : null,
                            gitStatus && gitStatus.ok
                              ? gitStatus.changed.slice(0, 12).map((c2) =>
                                  jsxRuntime.jsx(
                                    "button",
                                    {
                                      style: { ...STYLES.chip, width: "auto", padding: "3px 10px", fontSize: 11, cursor: "pointer", borderColor: gitDiff && gitDiff.file === c2.path ? "var(--sonar)" : undefined },
                                      title: "show this file's diff",
                                      onClick: () => showGitDiff(c2.path, c2.xy[0] !== "?" && c2.xy[0] !== " "),
                                      children: c2.xy + " " + c2.path,
                                    },
                                    c2.path,
                                  ),
                                )
                              : null,
                            gitStatus && gitStatus.ok && !gitStatus.clean
                              ? jsxRuntime.jsxs("span", { style: { marginLeft: "auto", display: "flex", gap: 8, alignItems: "center" }, children: [
                                  jsxRuntime.jsx("input", {
                                    style: { ...STYLES.input, width: 220 },
                                    placeholder: "commit message…",
                                    value: gitCommitMsg,
                                    onChange: (h) => setGitCommitMsg(h.target.value),
                                    onKeyDown: (h) => h.key === "Enter" && commitChanges(),
                                  }),
                                  jsxRuntime.jsx("button", { style: STYLES.ghostBtn, disabled: !gitCommitMsg.trim(), onClick: () => commitChanges(), children: "commit" }),
                                ] })
                              : null,
                          ],
                        }),
                        gitDiff
                          ? jsxRuntime.jsxs("div", { style: STYLES.projectHits, children: [
                              jsxRuntime.jsxs("div", { style: { display: "flex", gap: 8, alignItems: "center" }, children: [
                                jsxRuntime.jsx("span", { style: { color: "var(--kelp)", fontFamily: "'JetBrains Mono',monospace", fontSize: 11 }, children: "diff vs HEAD · " + gitDiff.file + (gitDiff.truncated ? " · cut" : "") }),
                                jsxRuntime.jsx("button", { style: STYLES.microBtn, title: "stage this file", onClick: () => stageFiles([gitDiff.file]), children: "stage this file" }),
                        ...
                        (i.pinned || []).length
                          ? jsxRuntime.jsxs("div", {
                              ...
                              children: [
                                ...
                                (i.pinned || []).map((p2) =>
                                  jsxRuntime.jsxs("span", { style: { display: "flex", gap: 6, alignItems: "center", ...STYLES.chip, width: "auto", padding: "3px 10px", fontSize: 11, ...(stalePins.includes(p2.id) ? { borderColor: "var(--amber)", color: "var(--amber)" } : {}) }, children: [
                                    p2.name + " · " + (p2.tokens || 0).toLocaleString() + " tok" + (p2.at ? " · " + String(p2.at).slice(5, 16).replace("T", " ") : "") + (stalePins.includes(p2.id) ? " · changed on disk" : ""),
                                    p2.path ? jsxRuntime.jsx("button", { style: STYLES.trayX, title: "re-read this file now", onClick: () => rePin(p2.id), children: "↻" }) : null,
                                    jsxRuntime.jsx("button", {
                                      style: STYLES.trayX,
                            ...
                            project
                              ? jsxRuntime.jsxs("span", {
                                  ...
                                  children: [
                                    ...
                                    " off-peak)",
                                    project.indexedAt ? " · indexed " + new Date(project.indexedAt).toTimeString().slice(0, 5) : "",
                                  ],
                                })
                              : jsxRuntime.jsx("input", {
                              ...
                              ? jsxRuntime.jsxs(jsxRuntime.Fragment, {
                                  children: [
                                    jsxRuntime.jsx("button", {
                                      style: STYLES.ghostBtn,
                                      disabled: projBusy,
                                      title: "walk the folder again — the map picks up new and changed files",
                                      onClick: () => indexProject(project.root),
                                      children: projBusy ? "indexing…" : "re-index",
                                    }),
                                    jsxRuntime.jsx("button", {
                                      style: STYLES.ghostBtn,
